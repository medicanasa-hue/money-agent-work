...
<p align="center">
  <a href="docs/openmontage_bridge.md">Use OpenMontage with MPT</a> for local animated explainers when stock footage is too generic.
</p>
...
Before a long render, verify the configured video encoder with a short local check:

```shell
uv run python cli.py --check-video-encoder
```

It reports the configured codec and the codec actually used. Exit code `2` means
the check completed with a `libx264` fallback.
