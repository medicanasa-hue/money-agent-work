...
def _resolve_listen_host(app_config: dict, root_config: dict) -> str:
    return app_config.get("listen_host", root_config.get("listen_host", "0.0.0.0"))


_CRITICAL_RUNTIME_CONFIG_KEYS = ("listen_host", "listen_port", "api_key")


def _validate_runtime_config(app_config: dict, root_config: dict) -> tuple[str, int]:
    """Validate the small set of settings that control the network boundary.

    Most configuration remains intentionally permissive for backwards
    compatibility. These settings are different: a typo can silently expose
    the API on every network interface, or prevent the server from starting.
    """
    if not isinstance(app_config, dict):
        raise ValueError("[app] configuration must be a TOML table")

    for config_scope in (app_config, root_config):
        for expected_key in _CRITICAL_RUNTIME_CONFIG_KEYS:
            if expected_key in config_scope:
                continue
            ...
            if matches:
                raise ValueError(
                    f"unknown critical setting '{matches[0]}'; "
                    f"did you mean '{expected_key}'?"
                )

    listen_host = _resolve_listen_host(app_config, root_config)
    if not isinstance(listen_host, str) or not listen_host.strip():
        raise ValueError("listen_host must be a non-empty string")

    listen_port = app_config.get("listen_port", root_config.get("listen_port", 8080))
    ...
    return listen_host.strip(), listen_port
...
listen_host, listen_port = _validate_runtime_config(app, _cfg)
