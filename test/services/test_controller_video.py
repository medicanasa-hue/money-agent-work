...
class TestVideoControllerTasks(unittest.TestCase):
    ...
    def test_delete_selects_task_directory_from_scanned_entries(self):
        """The recursive-delete path must originate from the task root listing."""
        task_id = "11111111-1111-4111-8111-111111111111"
        completed_task = {
            "task_id": task_id,
            "state": const.TASK_STATE_COMPLETE,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            current_task_dir = tasks_dir / task_id
            ...
            with (
                ...
                patch.object(
                    ...
                    side_effect=AssertionError(
                        "recursive delete must not derive its path from request data"
                    ),
                ),
        ...
        self.assertEqual(response["status"], 200)
        remove_tree.assert_called_once_with(os.path.realpath(current_task_dir))
        delete_task.assert_called_once_with(task_id)
    ...
    def test_delete_rejects_task_directory_that_resolves_to_another_task(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            tasks_dir = Path(temp_dir, "tasks")
            tasks_dir.mkdir()
            listed_path = os.path.abspath(tasks_dir / task_id)
            redirected_path = os.path.abspath(tasks_dir / "another-task")

            entry = MagicMock()
            entry.name = task_id
            entry.path = listed_path
            entry.is_dir.return_value = True
            entries = MagicMock()
            entries.__enter__.return_value = iter([entry])

            def redirect_matching_entry(path):
                absolute_path = os.path.abspath(path)
                if os.path.normcase(absolute_path) == os.path.normcase(listed_path):
            ...
            with (
                ...
                patch.object(video_controller.os, "scandir", return_value=entries),
                patch.object(
                    video_controller.os.path,
                    "realpath",
                    side_effect=redirect_matching_entry,
                ),
                patch.object(video_controller.shutil, "rmtree") as remove_tree,
                patch.object(video_controller.sm.state, "delete_task") as delete_task,
            ):
