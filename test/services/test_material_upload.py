...
class TestMaterialUploadService(unittest.TestCase):
    def test_sanitize_filename_strips_paths_and_validates_complete_extension(self):
        ...
        for filename in (
            "", ".", "..", "photojpg", "clip.mp4\x00", "clip.exe",
            "clip:stream.mp4", "clip?.mp4", ".material-upload-stage.mp4",
            "clip\u2028injected.mp4", "clip\u2029injected.mp4", "clip\x7f.mp4",
        ):
    ...
    def test_video_validation_disallows_playlists_and_network_protocols(self):
        with (
            patch.object(material_upload.Image, "open", side_effect=UnidentifiedImageError),
            patch.object(material_upload.utils, "get_ffmpeg_binary", return_value="ffmpeg"),
            patch.object(material_upload.subprocess, "run",
                         return_value=SimpleNamespace(returncode=0)) as run,
        ):
            material_upload._validate_video("/tmp/untrusted.mp4")
        ...
        self.assertEqual(command[command.index("-format_whitelist") + 1],
                         "mov,avi,flv,matroska,webm")
