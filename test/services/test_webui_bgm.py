...
class TestWebuiBackgroundMusic(unittest.TestCase):
    def setUp(self):
        self._original_ui_config = dict(config.ui)

    def tearDown(self):
        config.ui.clear()
        config.ui.update(self._original_ui_config)
    ...
    def test_sonilo_connection_button_reports_success(self):
        test_config = dict(config.app, sonilo_api_key="test-key")
        with (
            patch.object(config, "app", test_config),
            patch.object(config, "save_config"),
    ...
    def test_elevenlabs_source_reuses_masked_tts_key_and_shows_prompt(self):
        ...
        for locale in TEST_LOCALES:
            with self.subTest(locale=locale):
                ...
                self.assertEqual(api_key_input.value, "test-key")
                self.assertEqual(
                    api_key_input.label,
    ...
    def test_elevenlabs_connection_button_reports_success(self):
        test_config = dict(config.elevenlabs, api_key="test-key")
        with (
            patch.object(config, "elevenlabs", test_config),
            patch.object(config, "save_config"),
            patch.object(
    ...
    def test_elevenlabs_connection_reports_paid_plan_requirement(self):
        ...
        for locale in TEST_LOCALES:
            with self.subTest(locale=locale):
                test_config = dict(
                    config.elevenlabs, api_key="test-key"
                )
                with (
                    patch.object(config, "elevenlabs", test_config),
                    patch.object(config, "save_config"),
