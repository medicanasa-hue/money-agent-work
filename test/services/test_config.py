...
class TestRuntimeConfig(unittest.TestCase):
    def test_resolve_listen_host_prefers_app_setting(self):
        resolved = config._resolve_listen_host(
    ...
    def test_resolve_listen_host_keeps_legacy_top_level_setting(self):
        resolved = config._resolve_listen_host({}, {"listen_host": "localhost"})

        self.assertEqual(resolved, "localhost")

    def test_resolve_listen_host_uses_network_default_when_unspecified(self):
        self.assertEqual(config._resolve_listen_host({}, {}), "0.0.0.0")

    def test_validate_runtime_config_uses_app_network_settings(self):
        resolved = config._validate_runtime_config(
            {"listen_host": "127.0.0.1", "listen_port": 8088},
            {},
        )

        self.assertEqual(resolved, ("127.0.0.1", 8088))

    def test_validate_runtime_config_rejects_likely_listen_host_typo(self):
        with self.assertRaisesRegex(ValueError, "listen_hosts"):
            config._validate_runtime_config({"listen_hosts": "127.0.0.1"}, {})

    def test_validate_runtime_config_rejects_invalid_listen_port(self):
        with self.assertRaisesRegex(ValueError, "listen_port"):
            config._validate_runtime_config({"listen_port": 70000}, {})

    def test_validate_runtime_config_rejects_likely_api_key_typo(self):
        with self.assertRaisesRegex(ValueError, "api_kei"):
            config._validate_runtime_config({"api_kei": "local-only"}, {})
